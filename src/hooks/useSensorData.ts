...
// Interface pour les données brutes des capteurs dans Firebase
...
// Interface pour les données traitées et l'état de l'hook
 ...
 * @returns Données des capteurs et états de chargement/erreur
 */
export function useSensorData(salleId: string | null): SensorDataState {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(sensorRef, (snapshot: DataSnapshot) => {
      const data = snapshot.val() as SensorDataRaw | null;
  ...
  const isInNormalRange = (type: 'temperature' | 'humidity' | 'airQuality', value: number | null): boolean => {
